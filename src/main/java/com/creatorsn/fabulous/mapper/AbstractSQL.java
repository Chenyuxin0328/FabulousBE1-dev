...
public abstract class AbstractSQL<T> {
    ...
    public T OUTPUT_INSERT(String... columns) {
        sql().outputsInserted.addAll(Arrays.asList(columns));
        return getSelf();
    }

    public T OUTPUT(String... columns) {
        sql().outputs.addAll(Arrays.asList(columns));
        return getSelf();
    }

    public T OUTPUT_DELETE(String... columns) {
        sql().outputsDeleted.addAll(Arrays.asList(columns));
        return getSelf();
    }
    ...
    private static class SQLStatement {
        ...
        List<String> outputsInserted = new ArrayList<>();
        List<String> outputsDeleted = new ArrayList<>();
        ...
        private void sqlClause(SafeAppendable builder, String keyword, List<String> parts, String open, String close,
            ...
            if (!parts.isEmpty()) {
                ...
                for (int i = 0, n = parts.size(); i < n; i++) {
                    ...
                    if (prefix != null && !part.equals(AND) && !part.equals(OR) && !last.equals(AND) && !last.equals(OR)) {
        ...
        private String selectSQL(SafeAppendable builder) {
            ...
            if (distinct) {
                sqlClause(builder, "SELECT DISTINCT", select, "", "", ", ", null);
            } else {
                sqlClause(builder, "SELECT", select, "", "", ", ", null);
            }

            sqlClause(builder, "FROM", tables, "", "", ", ", null);
            joins(builder);
            sqlClause(builder, "WHERE", where, "(", ")", " AND ", null);
            sqlClause(builder, "GROUP BY", groupBy, "", "", ", ", null);
            sqlClause(builder, "HAVING", having, "(", ")", " AND ", null);
            sqlClause(builder, "ORDER BY", orderBy, "", "", ", ", null);
            limitingRowsStrategy.appendClause(builder, offset, limit);
            if (exists) {
                builder.append(") THEN 1 ELSE 0 END AS RESULT");
            }
        ...
        private String insertSQL(SafeAppendable builder) {
            sqlClause(builder, "INSERT INTO", tables, "", "", "", null);
            sqlClause(builder, "", columns, "(", ")", ", ", null);
            if (!outputsInserted.isEmpty()) {
                sqlClause(builder, "OUTPUT", outputsInserted, "", "", ", ", "inserted.");
            } else if (!outputs.isEmpty()) {
                sqlClause(builder, "OUTPUT", outputs, "", "", ", ", null);
            }
            for (int i = 0; i < valuesList.size(); i++) {
                sqlClause(builder, i > 0 ? "," : "VALUES", valuesList.get(i), "(", ")", ", ", null);
            }
            if (exists) {
        ...
        private String deleteSQL(SafeAppendable builder) {
            sqlClause(builder, "DELETE FROM", tables, "", "", "", null);
            if (!outputsDeleted.isEmpty()) {
                sqlClause(builder, "OUTPUT", outputsDeleted, "", "", ", ", "deleted.");
            } else if (!outputs.isEmpty()) {
                sqlClause(builder, "OUTPUT", outputs, "", "", ", ", null);
            }
            sqlClause(builder, "WHERE", where, "(", ")", " AND ", null);
        ...
        private String updateSQL(SafeAppendable builder) {
            sqlClause(builder, "UPDATE", tables, "", "", "", null);
            joins(builder);
            sqlClause(builder, "SET", sets, "", "", ", ", null);
            if (!outputsInserted.isEmpty() || !outputsDeleted.isEmpty() || !outputs.isEmpty()) {
                ArrayList<String> allOutputs = new ArrayList<>(outputsDeleted.stream().map(s -> "deleted." + s).toList());
                allOutputs.addAll(outputsInserted.stream().map(s -> "inserted." + s).toList());
                allOutputs.addAll(outputs);
                sqlClause(builder, "OUTPUT", allOutputs, "", "", ", ", null);
            }
            sqlClause(builder, "WHERE", where, "(", ")", " AND ", null);
            limitingRowsStrategy.appendClause(builder, null, limit);
            if (exists) {
